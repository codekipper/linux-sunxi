 ...
 */
static void sun4i_hdmi_write_infoframe(struct drm_encoder *encoder,
				     union hdmi_infoframe *frame)
{
}


static int sun4i_hdmi_set_audio_infoframe(struct sun4i_hdmi *hdmi)
{
	union hdmi_infoframe frame;
	int ret;
	...
	sun4i_hdmi_write_infoframe(&hdmi->encoder, &frame);
...
static void hdmi_audio_shutdown(struct device *dev, void *data)
{
	...
	sun4i_hdmi_set_audio_infoframe(hdmi);
}
...
static int hdmi_audio_configure(struct sun4i_hdmi *hdmi)
{
	...
	return sun4i_hdmi_set_audio_infoframe(hdmi);
}
...
static int sun4i_hdmi_audio_eld_ctl_info(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_info *uinfo)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct sun4i_hdmi *hdmi = snd_component_to_hdmi(component);
...
static int sun4i_hdmi_audio_eld_ctl_get(struct snd_kcontrol *kcontrol,
...
static const struct snd_soc_component_driver sun4i_hdmi_audio_component_drv = {
...
#else
{
	...
	ret = devm_snd_soc_register_component(dev, &sun4i_hdmi_audio_component_drv,
	...
	dai_link->codecs->dai_name = sun4i_hdmi_audio_codec_dai_drv.name;
