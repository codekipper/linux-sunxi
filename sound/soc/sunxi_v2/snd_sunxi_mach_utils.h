...
#define sunxi_simple_parse_cpu(node, dai_link,				\
			      list_name, cells_name, is_single_link)	\
	sunxi_simple_parse_dai(node, dai_link->cpus,			\
			      list_name, cells_name, is_single_link)

#define sunxi_simple_parse_codec(node, dai_link,				\
				list_name, cells_name)			\
	sunxi_simple_parse_dai(node, dai_link->codecs,			\
			      list_name, cells_name, NULL)

#define sunxi_simple_parse_platform(node, dai_link,			\
				   list_name, cells_name)		\
	sunxi_simple_parse_dai(node, dai_link->platforms,		\
			      list_name, cells_name, NULL)

struct sunxi_simple_dai {
	const char *name;
	unsigned int sysclk;
	int clk_direction;
	int slots;
...
struct sunxi_simple_data {
	u32 convert_rate;
	u32 convert_channels;
};

struct sunxi_simple_jack {
	struct snd_soc_jack jack;
	struct snd_soc_jack_pin pin;
	struct snd_soc_jack_gpio gpio;
};

struct sunxi_simple_priv {
	struct snd_soc_card snd_card;
	struct simple_dai_props {
		struct sunxi_simple_dai *cpu_dai;
		struct sunxi_simple_dai *codec_dai;
		struct snd_soc_dai_link_component cpus;   /* single cpu */
		struct snd_soc_dai_link_component codecs; /* single codec */
		struct snd_soc_dai_link_component platforms;
		struct sunxi_simple_data adata;
	...
	struct sunxi_simple_jack hp_jack;
	struct sunxi_simple_jack mic_jack;
...
int sunxi_simple_parse_daistream(struct device_node *node,
				char *prefix,
				struct snd_soc_dai_link *dai_link);
int sunxi_simple_parse_daifmt(struct device_node *node,
			     ...
			     char *prefix,
			     unsigned int *retfmt);
int sunxi_simple_parse_tdm_slot(struct device_node *node,
			       char *prefix,
			       struct sunxi_simple_dai *dais);
...
int sunxi_simple_parse_card_name(struct snd_soc_card *card, char *prefix);
